...
install-dev: pkg-dev
	f="$$(find . -iname "dmenu-launcher-dev-[a-f0-9.]*-x86_64.pkg.tar.zst" | grep -v "debug" | head -n 1)" && sudo pacman -U "$$f"

install-release: pkg-release
	f="$$(find . -iname "dmenu-launcher-[0-9.]*-x86_64.pkg.tar.zst" | grep -v "debug" | head -n 1)" && sudo pacman -U "$$f"
